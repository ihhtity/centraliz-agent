...
type Merch struct {
	...
	Account   string     `json:"account" gorm:"size:50;comment:账号"`                                  // 账号
	Password  string     `json:"password" gorm:"size:255;comment:密码"`                                // 密码
	Email     string     `json:"email" gorm:"size:100;comment:邮箱"`                                   // 邮箱
	Phone     string     `json:"phone" gorm:"size:20;comment:手机号"`                                   // 手机号
	Role      string     `json:"role" gorm:"size:20;default:'商家';comment:角色 商家 管理者 代理商"`             // 角色 商家 管理者 代理商
	Status    string     `json:"status" gorm:"size:20;default:'0';comment:状态 0白名单 1黑名单"`             // 0白名单 1黑名单
	LogAt     *time.Time `json:"logAt" gorm:"column:log_at;precision:3;default:null;comment:上次登录时间"` // 上次登录时间
