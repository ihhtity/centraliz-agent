...
type HuifuAccount struct {
	...
	Code      string    `json:"code" gorm:"size:255;default:'';comment:汇付编码"`  // 汇付编码
	Sharing   string    `json:"sharing" gorm:"comment:多方分账"`                   // 多方分账，text类型
	...
	Area      string    `json:"area" gorm:"type:text;comment:经营地址"`            // 经营地址
	Picture   string    `json:"picture" gorm:"type:text;comment:店铺图片"`         // 店铺图片
	Remarks   string    `json:"remarks" gorm:"type:text;comment:使用场景描述"`       // 使用场景描述，text类型
	Type      string    `json:"type" gorm:"size:10;comment:账号类型"`              // 账号类型
	Choose    string    `json:"choose" gorm:"default:'0';comment:0未选择 1已选择'"`  // 0未选择 1已选择
	Share     string    `json:"share" gorm:"default:'0';comment:0关闭分账 1开启分账'"` // 0关闭分账 1开启分账
